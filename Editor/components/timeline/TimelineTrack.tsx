...
export function TimelineTrack({ track, clips, pixelsPerSecond, selectedClipIds, isMuted, trimLimitId, onSelect, onMouseDown, onDoubleClick, onToggleVisibility, onToggleLock, onToggleMute }: TimelineTrackProps) {
  const meta = TRACK_META[track.type] ?? { icon: '•', label: (track.type[0] ?? 'V').toUpperCase() };
  const displayLabel = (track as any).label || `${meta.label}${(track.order ?? 0) + 1}`;
